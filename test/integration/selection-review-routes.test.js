...
test('selection reviews preserve inputs and pause at the correct next step', async () => {
  ...
  try {
    ...
    assert.deepEqual(input, { runId: 'fixture', combinedOpportunityReview: false, ...keywordBody });
    ...
    assert.deepEqual(input, { runId: 'fixture', combinedOpportunityReview: false, approvedKeywords: [], rejectedKeywords: [], manualKeywords: [], approveAll: false });
    ...
    runtime = { mode: 'keyword', activeStep: 'keywordReview', status: 'blocked', steps: ['start', 'verify', 'keywordReview', 'select'] };
    await post('keyword-review', keywordBody);
    assert.equal(input.combinedOpportunityReview, true);
    assert.equal(update.patch.activeStep, 'select');
    runtime.activeStep = 'select';
    assert.equal((await post('keyword-review', keywordBody)).status, 409);
