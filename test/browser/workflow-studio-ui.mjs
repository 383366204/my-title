...
try {
  ...
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 980, height: 800 }, { width: 390, height: 844 }]) {
    ...
    await page.getByRole('button', { name: '配置灵感', exact: true }).click();
    const configuration = page.getByRole('dialog');
    await configuration.getByText('高级设置', { exact: true }).click();
    ...
    await configuration.getByLabel('选词方向', { exact: true }).fill('水彩写生\n阳台园艺');
