...
const { generateTitles } = require('../../../../skills/title-gen/src/generate-title');
...
describe('generateTitles', () => {
  ...
  test('Test 1: Generated titles start with blue ocean keyword', async () => {
    // Mock LLMClient to return titles that should start with blueOceanWord
    const LLMClient = require('../../../../core/llm-client');
    const originalGenerateTitles = LLMClient.prototype.generateTitles;
    
    LLMClient.prototype.generateTitles = mock.fn(() => Promise.resolve([
    ...
    try {
      ...
      LLMClient.prototype.generateTitles = originalGenerateTitles;
    }
  });
  ...
  test('Test 2: Titles contain elements from 1688 original titles and taobao peer titles', async () => {
    ...
    // Mock LLM to return titles that include words from peer titles and products
    LLMClient.prototype.generateTitles = mock.fn(() => Promise.resolve([
  ...
  test('Test 4: Banned words are filtered from generated titles', async () => {
    ...
    // Mock LLM to return titles with banned words
    LLMClient.prototype.generateTitles = mock.fn(() => Promise.resolve([
    ...
    try {
      ...
      const bannedWords = require('../../../../skills/title-gen/data/banned-words.json');
  ...
  test('Test 5: Fallback titles still start with blue ocean keyword when LLM fails', async () => {
    const LLMClient = require('../../../../core/llm-client');
    const originalGenerateTitles = LLMClient.prototype.generateTitles;
    
    // Mock LLM to throw error (triggering fallback)
    LLMClient.prototype.generateTitles = mock.fn(() => Promise.reject(new Error('API failure')));
