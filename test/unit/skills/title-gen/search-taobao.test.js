...
beforeEach(() => {
  ...
  delete require.cache[require.resolve('../../../../skills/title-gen/src/search-taobao')];
});
...
test('searchTaobaoTitles caches by keyword and avoids duplicate native calls', async () => {
  ...
  const taobaoUtilsPath = require.resolve('../../../../skills/title-gen/src/taobao-utils');
...
test('guardTaobaoImageSearch caches image search result by image url', async () => {
  const dataDir = tempDataDir();
  let calls = 0;
  const { guardTaobaoImageSearch } = require('../../../../skills/title-gen/src/search-taobao-image');
