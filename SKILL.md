---
name: ecom-ai-tools
description: 电商选品AI工具箱。当用户需要生成淘宝/1688 商品标题、做电商 SEO 优化、根据关键词生成商品标题、查同行标题、做电商选品、查看1688商机热榜、趋势分析、查生意参谋数据时使用。基于 LLM 提取核心词 + 1688 实时搜索 + 同行标题参考，输出三段式 SEO 优化标题。
---
...
基于 **LLM + 1688 + 淘宝** 的电商标题自动生成 CLI 工具。输入一个关键词，输出符合淘宝 SEO 规则的三段式优化标题。
...
```
用户关键词
  ↓ LLM 提取核心词 + 判断修饰词刚性(rigid/optional)
  ↓ 并行调用：1688 AI 版搜索 + 淘宝同行搜索
  ↓ 刚性修饰词过滤（材质/颜色/人群必须匹配）
  ↓ 高频词提取 + 违禁词过滤
  ↓ LLM 三段式生成（核心词前置 → 刚性词 → 高频/可选词）
  ↓ 输出标题（默认 60 字符，可调）
```

## 前置条件

skill 路径：`~/.openclaw/workspace/skills/ecom-ai-tools/`
...
```json
{
  "mcpServers": {
    "ecom-ai-tools": {
      "command": "node",
      "args": ["~/.openclaw/workspace/skills/ecom-ai-tools/bin/mcp-server.mjs"],
      "timeout": 180000,
      "trust": "trusted"
    }
  }
...
- **LLM API 报错** → 自动降级到 `fallbackExtract()`，但效果会差
