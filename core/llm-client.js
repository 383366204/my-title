...
const PROMPT_VERSION = 4; // bump when LLM prompts change
...
class LLMClient {
  /**
   * 构造函数 — 仅接收已解析的配置，不直接读取环境变量。
   * 所有环境变量解析逻辑统一由 core/llm/index.js 的 createLLMClient() 工厂负责。
   *
   * @param {object} config - 已解析的 LLM 配置
   * @param {string} [config.apiKey] - API 密钥
   * @param {string} [config.apiBase] - API 基础地址
   * @param {string} [config.model] - 模型名称
   * @param {string} [config.provider] - 服务商标识
   * @param {number} [config.timeout] - 请求超时(ms)
   * @param {number} [config.longTimeout] - 长请求超时(ms)
   */
  constructor(config = {}) {
    ...
    // 超时：优先使用显式配置，否则根据 apiBase 推断合理默认值
    const defaultTimeout = this.apiBase.includes('volces.com') ? 30000 : 15000;
    this._timeout = parseInt(config.timeout, 10) || defaultTimeout;
...
[
  {"productId": "商品ID", "score": 8, "reason": "评分原因"}
    ...
    if (!Array.isArray(result)) {
      throw new Error('Invalid response format from LLM: expected array');
    }
 ...
 输出严格 JSON 格式，不要任何其他文本：
 {
   ...
   * RED: 测试阶段先通过 LLM 接口获取结果，JSON 结构如下：
   * {
...
- 避免堆砌：同类词选1个（如"锁骨链"和"颈链"只取搜索量更高的）`;
  ...
  输出严格 JSON 格式，不要任何其他文字，字段名必须完全一致：
  {
    ...
    if (!result || !Array.isArray(result.selectedProducts) || !Array.isArray(result.titles)) {
      throw new Error('Invalid response format from LLM selectAndGenerate');
    }
...
// 向后兼容：保留旧名称导出，避免外部引用立即断裂
module.exports.GLMClient = LLMClient;
