 ...
 * @param {Map<string, string>} baseline snapshotDraftRows 的结果
 * @returns {Array<{id:string, reviewContent:string}>} 有改动的行
 */
export function collectChangedReviews(rows = [], baseline = new Map()) {
  return (Array.isArray(rows) ? rows : [])
    ...
    .map(row => {
      ...
      const current = String(row.reviewContent || '');
      return baseline.get(id) === current ? null : { id, reviewContent: current };
    })
    .filter(Boolean);
}