...
const rows = [
  { id: 'a', title: '商品甲', experienceNotes: '真实体验一', reviewContent: '初稿一', correspondingFile: '' },
  { id: 'b', title: '商品乙', experienceNotes: '真实体验二', reviewContent: '初稿二', correspondingFile: '旧文件.png' }
];

test('snapshot builds the autosave baseline keyed by draft id', () => {
  const baseline = snapshotDraftRows(rows);
  assert.deepEqual(baseline.get('a'), { experienceNotes: '真实体验一', reviewContent: '初稿一', correspondingFile: '' });
  assert.deepEqual(baseline.get('b'), { experienceNotes: '真实体验二', reviewContent: '初稿二', correspondingFile: '旧文件.png' });
...
test('collects only rows whose review content or file changed', () => {
  ...
  assert.deepEqual(changed[0], { id: 'a', experienceNotes: '真实体验一', reviewContent: '人工改过的一', correspondingFile: '' });
});
...
test('keeps editing state dirty when content is cleared mid-edit', () => {
  ...
  assert.deepEqual(changed, [{ id: 'a', experienceNotes: '真实体验一', reviewContent: '', correspondingFile: '' }]);
});
...
test('saving mid-edit content back into the baseline stops further diffs', () => {
  ...
  for (const item of changed) baseline.set(item.id, { experienceNotes: item.experienceNotes, reviewContent: item.reviewContent, correspondingFile: item.correspondingFile });
