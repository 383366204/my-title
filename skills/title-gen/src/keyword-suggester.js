...
async function suggestKeywords(options) {
  ...
  if (strategy === STRATEGIES.TREND) {
    try {
      if (typeof fetchHotData === 'function') {
        const opportunities = await fetchHotData();
...
async function verifyKeywordsWithSycm(candidates, options = {}) {
  ...
  if (typeof verifyFn !== 'function') {
    return {
      ...
      errors: [],
      message: '未提供 SYCM 验证回调，跳过验证'
    };
  }

  // 使用外部验证回调
  try {
    const result = await verifyFn(candidates, { port, delay, maxCandidates, onProgress });
    // 确保返回格式兼容
    ...
    if (Array.isArray(result)) {
      return {
        ok: true,
        keywords: result.map(item => typeof item === 'string' ? { keyword: item } : item),
        verified: result.length,
        failed: 0,
        errors: []
      };
    }
    throw new Error('验证回调返回格式无效');
  } catch (error) {
    return {
      ok: false,
      error: error.message,
      ...
      verified: 0,
      failed: candidates.length,
      errors: [{ keyword: 'all', error: error.message }]
    };
  }
