...
```
用户输入关键词 → GLM 提取核心词 → 1688 搜索相关商品 → 拼接生成符合淘宝SEO规范的标题（60字符）
```

### 1.2 核心价值

- **自动化**: 减少手动搜索和标题拼接的时间
- **标准化**: 输出格式统一（60 字符，符合淘宝SEO最佳实践）
...
- 标题长度限制：默认 60 字符（30 个中文），部分类目可达 128 字符
 ...
 */
function generateTitles(userInput, coreWord, searchTitles, maxLength = 60) {
  // 策略:
  // 1. 三段式结构: 核心词 + 属性词 + 场景词
  // 2. 从搜索结果提取高频属性词和场景词
  // 3. 过滤违禁词
...
| 规范项 | 要求 | 说明 |
|--------|------|------|
| **字数限制** | 默认 60 字符（30 中文），部分类目 128 字符 | 确保搜索权重最高区域完整 |
...
| 规则项 | 规范要求 | 违规后果 |
|--------|----------|----------|
| 标题长度 | 默认 60 字符（30 中文），部分类目 128 字符 | 超长部分被截断，影响展示 |
